...
export const upsertTipoCambio = async (req: Request, res: Response) => {

  console.log('Cuerpo de la solicitud:', req.body); 
  const { nombre_empresa, nombre_bd, habilitada } = req.body;
  // Validar parámetros obligatorios
  if (!nombre_empresa || !nombre_bd || typeof habilitada !== 'boolean') {
    return res.status(400).json({ error: 'Faltan datos obligatorios: nombre_empresa, nombre_bd y habilitada (boolean)' });
  }
  try {
    ...
    let queryValues: any[] = [];
    if (habilitada) {
  queryText = `INSERT INTO TipoCambio (empresa_id, basedatos_id, habilitada, fecha_alta, fecha_baja)
  ...
  queryText = `UPDATE TipoCambio SET habilitada = false, fecha_baja = NOW() WHERE empresa_id = $1 AND basedatos_id = $2 RETURNING *`;
      queryValues = [empresa_id, basedatos_id];
    }
  ...
  res.status(500).json({ error: 'Error al guardar tipocambio' });
  }
};

// Buscar tipocambio
export const buscarTipoCambio = async (req: Request, res: Response) => {
  ...
  if (!nombre_empresa && !rut && !nombre_bd && !n_sistema && !n_instalacion) {
    return res.status(400).json({ error: 'Debe enviar al menos un parámetro de búsqueda' });
  }
  try {
  let query = `SELECT t.*, e.nombre_empresa, e.rut, e.n_sistema, e.n_instalacion, b.nombre_bd
         FROM TipoCambio t
         JOIN Empresa e ON t.empresa_id = e.id
         JOIN BaseDatos b ON t.basedatos_id = b.id
         WHERE 1=1`;
    const params: any[] = [];
    if (nombre_empresa) {
  ...
  res.status(500).json({ error: 'Error al buscar tipocambio' });
  }
};
