...
// Crear o actualizar interfaz
export const upsertInterfaz = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let queryValues: any[] = [];
    if (habilitada) {
      queryText = `INSERT INTO Interfaz (empresa_id, basedatos_id, habilitada, fecha_alta, fecha_baja)
      ...
      queryText = `UPDATE Interfaz SET habilitada = false, fecha_baja = NOW() WHERE empresa_id = $1 AND basedatos_id = $2 RETURNING *`;
      queryValues = [empresa_id, basedatos_id];
    }
    ...
    res.status(500).json({ error: 'Error al guardar interfaz' });
  }
};

// Buscar interfaz
export const buscarInterfaz = async (req: Request, res: Response) => {
  const { nombre_empresa, rut, nombre_bd, n_sistema, n_instalacion } = req.query;
  try {
    let query = `SELECT i.*, e.nombre_empresa, e.rut, e.n_sistema, e.n_instalacion, b.nombre_bd
                 FROM Interfaz i
                 JOIN Empresa e ON i.empresa_id = e.id
                 JOIN BaseDatos b ON i.basedatos_id = b.id
                 WHERE 1=1`;
    const params: any[] = [];
    if (nombre_empresa) {
    ...
    res.status(500).json({ error: 'Error al buscar interfaz' });
  }
};
